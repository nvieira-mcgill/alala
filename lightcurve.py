...
class LightCurve:
    def __init__(self, mag_tab=None, lim_mag_tab=None, ref_mag_tab=None):
        ...
        self.plot_instructions_lim_mags = {"u":["#9a0eea","v"], 
                                           "g":["green","v"],
                                           "r":["#ff2100","v"],
                                           "i":["#ffb07c","v"],
                                           "z":["#ff796c","v"],
        ...
        self.plot_instructions_ref_mags = {"u":"#9a0eea", 
                                           "g":"green",
                                           "r":"#ff2100",
                                           "i":"#ffb07c",
                                           "z":"#ff796c",
    ...
    def __str__(self):
        ...
        if len(self.mags) > 0: self.mags.pprint()
        if len(self.lim_mags) > 0: self.lim_mags.pprint()
        if len(self.ref_mags) > 0: self.ref_mags.pprint()            
        return ""
    ...
    def plot(self, *filters, output="lightcurve.png", title=None, 
        ...
        Input: 
            ...
            - value in MJD for the time of merger (or some general reference 
              time which should be considered t=0) to plot time elapsed t=0
              (optional; default None, which just plots MJD)
        ...
        if tmerger:
            plt.xlabel(r"$T - T_{merger}$"+" [days]", fontsize=18)
        else:
